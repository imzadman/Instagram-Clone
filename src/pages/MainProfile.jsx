...
export function MainProfile() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        console.log(error.message || "Failed to fetch profile");
        setLoading(false);
      }
    })();
    //Fetching userPosts:
    (async () => {
      if (userData && userData.$id) {
        const get = await dbService.getUserPosts(userData.$id);
        if (get) {
    ...
    (async () => {
      const { followersCount, followingCount } =
        await profileService.getFollowCount(userData.$id);
  ...
  const fetchingFollowersProfile = async () => {
    try {
      const followers = await profileService.getFollowers(userData.$id);
      const profiles = [];
  ...
  const fetchingFollowingProfiles = async () => {
    try {
      const following = await profileService.getFollowing(userData.$id);
      const profiles = [];
                ...
                {isOwner && (
                  <>
                    ...
                    <Link to={`/edit-profile/${profile.$id}`}>
                      <Button
                        type="submit"
                        className="bg-black dark:bg-green-600 text-gray-50 rounded"
                      >
